...
to go
  ...
  ask turtles[
    if bird-groups != 0 and count turtles with [bird-group-index = group_indx] >= birds-in-groups-threshold [
      set groups_lst remove-duplicates (insert-item 0 groups_lst group_indx)
    ]
  ]
...
to find-bird-groups
 ask turtles [
  ...
  ask turtles [
    ...
    let idx bird-group-index

    ask bird-groups [set group_indx idx]
    set current-group-index group_indx]
    ]
  ]

   ask turtles [
      set label group_indx
      set color red
    set size 3
  ]
...
to get-groups
  ifelse count turtles in-radius (2 * minimum-separation) >= birds-in-groups-threshold [
...
to-report average-group-heading
  let x-component sum [dx] of turtles with [group_indx = current-group-index]
  let y-component sum [dy] of turtles with [group_indx = current-group-index]
  ifelse x-component = 0 and y-component = 0
    [ report heading ]
    [ report atan x-component y-component ]
end
...
to find-deviation-group-heading
  ...
  if length ([heading] of turtles with [group_indx = current-group-index]) > 1 [
  set group-heading-deviation variance ([heading] of turtles with [group_indx = current-group-index])
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot length groups_lst"
...
<experiments>
  ...
  <experiment name="Birds-Groups-Count-Experiment" repetitions="50" runMetricsEveryStep="true">
    ...
    <steppedValueSet variable="vision" first="1" step="10" last="50"/>
    <steppedValueSet variable="minimum-separation" first="1" step="1" last="10"/>
    <enumeratedValueSet variable="population">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-align-turn">
