...
to go
  ...
  set bird-groups-qty length (remove-duplicates ([bird-index] of turtles))
...
to find-bird-groups
 ask turtles [
    get-groups
    ; индекс внешней птицы для которой ищем группы
    set bird-group-index who
    ; индекс птиц в группе
    set bird-index who
  ]
  ...
  ask turtles [
   ...
   ask bird-groups [set bird-index idx]
  ]

  ask turtles [
    set current bird-index
...
to get-groups
  ifelse count turtles in-radius (2 * minimum-separation) > 3 [set bird-groups n-of 3 turtles in-radius (2 * minimum-separation)]
  [set bird-groups turtles in-radius (2 * minimum-separation)]
  set count_birds_in_group count bird-groups
end
...
to-report average-group-heading
  let x-component sum [dx] of turtles with [bird-index = current]
  let y-component sum [dy] of turtles with [bird-index = current]
  ifelse x-component = 0 and y-component = 0
    [ report heading ]
    [ report atan x-component y-component ]
end
...
to find-deviation-group-heading
  ...
  ifelse length ([heading] of turtles with [bird-index = current]) > 1 [
    set group-heading-deviation variance ([heading] of turtles with [bird-index = current])]
    [set group-heading-deviation 0]
